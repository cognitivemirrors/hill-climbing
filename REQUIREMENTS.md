...
- **Revision history:**
  ...
  - v0.2 (inventory freshness) — added the **Writing** blog (`blog.html`, reached by a quiet footer link on the hub; precached in `sw.js`). **No Appendix A row is owed:** the blog is a fully static reading page — it holds no localStorage or IndexedDB, logs no usage, and makes no network request beyond fetching its own already-precached assets. It joins **no** data flow (no store, no sync blob, no egress), so it trivially passes §1's three tests. Noted here only so the inventory's *absence* of a blog entry is a recorded decision, not an omission.
