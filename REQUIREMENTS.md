...
- **Revision history:**
  ...
  - v0.6 (**founder-ratified — cross-user Contacts + messaging**) — the founder **invoked and granted the CONSTRAINTS "two only / product-shape" re-ratification** (§5 world anti-patterns; asked explicitly before building) to ship the suite's **first cross-user surface**: `contacts.html` v0.1 (contact list + human messaging + friend graph) over a new Supabase backend (`contacts-schema.sql`: `profiles`/`friendships`/`messages` + RLS + `security definer` RPCs). Scope as ratified: human↔human + human↔own-agent messaging only — **no cross-user *portrayed* characters, so CONSTRAINTS S8 is not triggered**. The **relational-safety gate** is met by **two-sided consent + blocking + exit enforced in the database**; the **crypto gate** is met honestly under the public-data frame — messages are **public seed data, server-readable, not E2EE**, disclosed in-thread, pair-readable via RLS (no false confidentiality). Appendix A gains the `hill-climbing-contacts` local row (A.1), an A.4 cross-user egress bullet (the first operator-readable user-to-user flow), and the `ls:hill-climbing-contacts` blob (A.5). Binding cross-user commitment, recorded here and in CONSTRAINTS. **Remaining before it is relied on:** live two-account verification; an abuse-report path; message-E2EE only if confidential DMs are ever wanted (they contradict the frame). (KNOWN_RISKS L50.)
...
| `hill-climbing-contacts` | Contacts **local** state: `{ v, handle, introduced [{id,name,note,created}], agentChats {contactKey → [{role,text,ts}]} }` — your cached handle, agents you've noted being introduced to, and on-device conversations with family/introduced agents. Personal (agent conversations can be intimate). **Synced (E2EE) when sync is on** (A.5). **Human friends + their messages are NOT in this store — they live server-side** (`public.profiles`/`friendships`/`messages`; see A.4 + `contacts-schema.sql`). | grows with use | Until user clears (local); server rows via unfriend/block | In-app "Delete local contacts data"; server rows removed on unfriend/block |
...
- **Opt-in cross-user messaging (Contacts, `contacts.html` v0.1; the suite's FIRST user-to-user flow; founder-ratified the product-shape crossing — CONSTRAINTS "two only" clause, 2026-07).** When signed in, a person can claim a handle, invite/accept friends, and exchange messages with accepted friends. The friend graph + messages leave the device to the operator's Supabase DB (`profiles`/`friendships`/`messages`). Under the public-data frame these messages are **public seed data — server-readable, NOT end-to-end encrypted** — and the thread discloses this plainly; they are readable only by the two parties (RLS). This is the **first operator-readable user-to-user content flow** (unlike E2EE sync, the operator *can* read it — disclosed, not a privacy failing under the frame). **Relational safety is enforced in the DB, not the UI:** RLS + `security definer` RPCs require mutual acceptance before any message and make a block stop contact both ways (`contacts-schema.sql`). Messaging your *own* family/introduced agents is a BYOK model call (agent side), authored on-device and server-unread. No abuse-report path yet (follow-up). Metadata (who-messages-whom, handles) is operator-visible.
