...
| `hill-climbing-reflect-photo-ai` | Reflect photo-describe opt-in flag (`'1'` = enabled). When enabled (and only then), each **newly attached** journal photo is sent once to Anthropic on the user's own key to produce a description + text transcription — see A.4. Device-local UI preference; **never synced** (each device consents separately). | ~5 B | Until user clears or turns the toggle off in-app | In-app toggle ("describe photos: on" → tap to turn off); browser settings |
