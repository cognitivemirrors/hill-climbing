...
|---|---|---|---|
| **BYOK AI apps** — Council, Companion, Nourish chef, and the hub's companion next-step | The text/decision the user submits, or the goals + journal + activity digest Companion assembles; the hub's next-step sends a **narrower** digest (activity day-counts/durations, open in-progress items, the focused Climb goal + open steps, and the companion's memory summary — **no journal prose**) — all on the **user's own Anthropic key** | **Anthropic**, under the user's own API terms — **never the operator** (there is no operator server in the path) | User owns the key and the account; explicit key gate; plain-language disclosure; Companion shows the verbatim digest before it sends (L24/L27/L29). The hub next-step is the one flow that fires **on arrival rather than per user action** — which is why it is additionally gated behind its own explicit on-hub enable that names exactly what is sent (L40) |
...
| **WHOOP integration (Recover)** | The user's own WHOOP developer-app credentials (client ID/secret + rotating OAuth tokens) and read-only data requests; WHOOP's replies (recovery, HRV, RHR, sleep, strain, workout counts) come back down the same pipe and are stored on-device | **WHOOP** (the user's own vendor account), transiting a **stateless relay the operator runs** (`whoop-relay`, a Supabase Edge Function) — the suite's first operator-run hop in a user-content path. The relay stores nothing, logs nothing, and holds no secrets | Bring-your-own developer app (the WHOOP analog of BYOK); relay pinned to `api.prod.whoop.com` read-only paths; credentials/tokens device-local, **never synced**; metrics on-device + E2EE-synced; after the explicit connect, re-sync fires quietly on open (throttled ≥ 6 h) — recurring-pattern consent collected up front, like the hub next-step (L41) |
...
- **Free-form personal prose (highest).** Reflect journal entries; Council deliberations; Companion conversations; Levity notebook lines; ERP exposure-ladder fears and session reflections (`hill-climbing-erp`). On-device; end-to-end encrypted if sync is on. Council and Companion additionally send some of this to Anthropic on the user's own key when the user acts (Companion sends the journal digest by design on every reply — L29). Companion also persists a **model-written rolling summary of the user's past conversations** (its cross-conversation memory, added v0.8 — L37): the first *distilled* at-rest personal content in the suite, held in the same `hill-climbing-companion` blob and E2E-encrypted with it.
- **Daily physiological metrics (health data).** Recover's WHOOP-derived day map (`hill-climbing-whoop`): recovery %, HRV, resting heart rate, SpO2, sleep duration/performance, respiratory rate, strain, workout counts. Not prose, but a health record — more revealing than practice flags, and a new sensitivity class for the suite (recover v0.1, L41). On-device; E2EE-synced when sync is on. Compact summaries also ride the Companion and hub-next-step digests to Anthropic when Recover is connected (disclosed in both consent notes).
...
- **Credential.** The single shared Anthropic API key (`hill-climbing-api-key`), and the WHOOP developer-app credentials + OAuth tokens (`hill-climbing-whoop-auth`). Device-local, **never synced.** Browser localStorage is not a secure secret store — an accepted Tier-0 risk (L29, L41).
...
- **Tier 1 (friends & family):** before anyone else uses an app that can leave the device, they get a plain-language account of *what* leaves and *to whom* (the five flows in §1.1). Informal — a conversation or a short in-product note — but explicit and before first use.
...
- **BYOK AI apps (all tiers, opt-in):** each off-device send is user-initiated and gated behind an explicit key setup that names the data flow and Anthropic in plain language; billed to the user's own account; Nourish's chef adds its own first-run consent checkbox. **One deliberate exception to "user-initiated per send" (founder-directed):** the hub's companion next-step, once explicitly enabled on the hub, sends its digest automatically each time the user returns to the hub (throttled). The consent for the *recurring* pattern is collected up front — the enable note states that it fires on every return — and turning it off is a single tap (L40).
- **WHOOP integration (Recover, all tiers, opt-in):** connecting is explicit and deliberate — the user creates their own WHOOP developer app, pastes its credentials into Recover, and approves the scopes on WHOOP's own consent screen; the in-app note names the relay and exactly what travels where before the first connect. Once connected, Recover refreshes quietly on open (throttled ≥ 6 h) — consent for the *recurring* pattern is collected up front, the same shape as the hub next-step. Disconnect (forget credentials) and delete (erase metrics) are each one tap.
...
| Off-device egress limited to the five opt-in flows in §1.1 (BYOK AI apps → `api.anthropic.com` only, incl. Companion's server-side web search/fetch; E2EE sync → ciphertext + metadata only; Web Push subscription; WHOOP via the pinned `whoop-relay` → `api.prod.whoop.com` only) — no analytics, telemetry, error reporting, or third-party scripts (detail: Appendix A.4) | Network-tab inspection during a full session | Every release |
...
| WHOOP relay stays a stateless pinned pipe (L41) | Review `supabase/functions/whoop-relay/index.ts`: no storage, no logging of bodies/tokens, upstream fixed to `api.prod.whoop.com`, path allowlist intact; confirm the deployed function matches the repo source | Every release touching the relay or `recover.html` |
| Recover consent copy names the full flow (credentials → relay → WHOOP; metrics on-device/E2EE; auto-resync cadence) | Read the connect-screen note against the actual code paths | Every release touching `recover.html` |
...
- **Revision history:**
  ...
  - v0.2 (founder-directed, recover v0.1) — **WHOOP integration.** §1.1 gains a fifth opt-in flow row (WHOOP via the stateless `whoop-relay` Edge Function); §1.2 gains the **health-data sensitivity class** and folds the WHOOP credentials into the credential class; §1.3's delete-all extended to the three `hill-climbing-whoop*` keys; §2.1 flow-count updated; §2.3 gains the WHOOP consent bullet; §5 gains relay-stateless + consent-copy verification rows and widens the egress row; A.1 gains the `hill-climbing-whoop` / `-auth` / `-state` rows; A.4 gains the relay flow paragraph and notes the Companion/hub digest widening; A.5 lists the new sync blob and the never-synced credential keys. **Posture change, founder-chosen (live API over CSV import, decided via AskUserQuestion this session):** the suite's first operator-run relay inside a user-content path — "no operator server in the path" now holds for the BYOK AI flows but *not* for WHOOP. Accepted at Tier 0 where operator = user; flagged as a Tier-1 disclosure item (KNOWN_RISKS **L41**).
...
| `hill-climbing-erp` | ERP (exposure & response prevention) state: `{ v, items {id → {title, situation, obsession, ritual, anticipated (0–100 SUDS), status active/resting, order, created, updated}}, sessions [{id, itemId, ts, preSUDS, peakSUDS, endSUDS, resisted full/partial/none, expected, happened, note, minutes, created}] }`. `items[].obsession`/`situation`/`ritual` and `sessions[].expected`/`happened`/`note` are **free-form personal prose about fears and feared outcomes** — journal-grade sensitivity (see L38). **Synced (E2EE) when sync is on** (A.5). | grows slowly | Until user clears or in-app Delete everything | In-app Delete everything; browser settings; future "delete all my data" button (Tier ≥ 2 prerequisite) |
...
| `hill-climbing-requests` | Companion **requests to the builder** (added v0.9): `{ v, items: [ { id, ts, heading, prompt } ] }` (cap 100). Requests **about the app suite itself**, captured from a conversation via the `add_request` client tool once the user okays it — each a short `heading` plus a `prompt` (what to build, or questions to answer) the person building this can act on. The user's own product feedback, not personal life content; `heading`/`prompt` are short user-derived text about the apps. On-device; the model that writes it runs on the user's own key, but the write itself is local (no new egress). **Synced (E2EE) when sync is on** (A.5). | grows slowly (cap 100) | Until user clears (in-app per-item delete or "Clear all", or browser clear) | In-app per-item delete / Clear all; browser settings; future "delete all my data" button (Tier ≥ 2 prerequisite) |
| `hill-climbing-whoop` | Recover metrics day-map: `{ v, days: { "YYYY-MM-DD": { rec, hrv, rhr, spo2, sleepMin, sleepPerf, resp, strain, wk, cal? } }, lastSync }` — daily recovery %, HRV (ms), resting HR, SpO2, sleep minutes + performance %, respiratory rate, day strain, workout count, calibrating flag. **Health data — a new sensitivity class** (§1.2, L41); no free text, no identity. Written by `recover.html` from the user's own WHOOP account; read by the hub next-step engine and Companion's digest (both on-device reads). **Synced (E2EE) when sync is on** (A.5). | grows ~35 KB/yr | Until user clears or in-app "Delete all Recover data" | In-app Delete all; browser settings; future "delete all my data" button (Tier ≥ 2 prerequisite) |
| `hill-climbing-whoop-auth` | The user's own **WHOOP developer-app credentials + OAuth tokens**: `{ cid, secret, access, refresh, exp }`. A credential — browser localStorage is not a secure secret store (accepted Tier-0 risk, same class as the Anthropic key; L41). Device-local; **never synced** — WHOOP rotates refresh tokens, so two devices sharing one would race; each device connects itself. | < 1 KB | Until user clears or in-app Disconnect | In-app Disconnect / Delete all; browser settings |
| `hill-climbing-whoop-state` | Transient OAuth CSRF `state` nonce for the in-flight WHOOP connect; written at redirect-out, removed at callback. Device-local; never synced. | ~10 B | One OAuth round-trip | Auto-removed at callback; browser settings |
...
- **Opt-in WHOOP integration (Recover; added recover v0.1; L41).** If — and only if — the user connects Recover to their own WHOOP developer app, the browser talks to WHOOP through **`whoop-relay`**, a stateless Supabase Edge Function the operator runs — the suite's **first operator-run hop inside a user-content path**. It exists only because WHOOP's OAuth token endpoint and developer API do not answer browsers directly. Per request, the user's client ID/secret or bearer token rides from the browser through the relay to `api.prod.whoop.com`, and the health data rides back down the same response; the relay stores nothing, logs nothing, holds no secrets of its own, and is pinned to WHOOP's OAuth path plus an allowlist of read-only v2 data paths (source: `supabase/functions/whoop-relay/index.ts`). **Honest limit:** "stores nothing, logs nothing" is an *operational* guarantee — code and configuration the operator could change — not a cryptographic one like E2EE sync. That distinction is recorded in L41 and is a named Tier-1 disclosure item. Returned metrics are stored on-device only (`hill-climbing-whoop`, E2EE-synced). After the explicit connect, Recover re-syncs quietly on open (throttled ≥ 6 h) — recurring-pattern consent collected up front by the connect note. Related widening: the Companion digest and the hub next-step digest now carry a compact WHOOP summary when Recover is in use — health data joining those Anthropic-bound sends; both in-product consent notes name it.
...
- **Blob (last-write-wins):** `ls:hill-climbing-climb` (Climb state, v1.99), `ls:hill-climbing-companion` (Companion conversations, v0.1), `ls:hill-climbing-requests` (Companion requests to the builder, v0.9), `ls:stillness-game` (Meditate staircase), `ls:breathe-session-duration` (Breathe session length), `ls:hill-climbing-nourish` (Nourish ladder), `ls:hill-climbing-savor` (Savor tasting-show state, incl. tasting notes + own lessons), `ls:hill-climbing-levity` (Levity state), `ls:hill-climbing-train` (Train state), `ls:hill-climbing-council` (Council deliberations), `ls:hill-climbing-erp` (ERP ladder + exposure record, v0.1), `ls:hill-climbing-whoop` (Recover WHOOP metrics day-map — health data, recover v0.1).
...
**Never registered (device-local, never synced):** the shared Anthropic credential `hill-climbing-api-key` (A.1), the WHOOP credentials/tokens `hill-climbing-whoop-auth` + the transient `hill-climbing-whoop-state` nonce (A.1), Meditate's `hill-combing-reports` + `hill-combing-last-trajectory`, all UI/onboarding flags, the usage **device id** + **backfill guard** (A.1), and the legacy `hill-climbing-usage` localStorage **aggregate** (a per-device convenience copy for companion/offline; the synced source of truth is the `hc-usage/records` log above, not this blob). What leaves the device for sync is always ciphertext + metadata only; content never does.
