...
| `hill-climbing-companion` | Companion saved conversations **and cross-conversation memory**: `{ v, conversations: [ { id, ts, title, thread [{ role, text, hidden? }] } ] (cap 60), memory: { summary, folded, ts } }`. `thread[].text` is **free-form personal conversation** — the user's own words plus the companion's replies, which may paraphrase their goals or journal. `memory.summary` (**added v0.6**) is a **model-written rolling distillation of the user's older conversations** — an evolving profile of what matters to them, which may paraphrase goals/journal content the companion surfaced; `memory.folded` maps conversation id → turns already distilled. Among the most sensitive content in the suite (see L29, **L32**). The raw goals/journal/activity **digest** the companion is shown at reply time is still *not* stored here; what persists is the dialogue **and the distilled memory of it**. **Synced (E2EE) when sync is on** (§1.5). | < 250 KB (cap 60 + bounded memory) | Until user clears (in-app "Clear all conversations", **which erases the memory too**) or in-app erase | In-app Clear all conversations (**dialogue + memory**); browser settings; future "delete all my data" button (Tier ≥ 2 prerequisite) |
...
| Companion cross-conversation memory stays bounded, erasable, and is distilled without web egress (L32) | Confirm `hill-climbing-companion.memory.summary` stays a bounded rolling summary (not an unbounded transcript pile); "Clear all conversations" empties `memory`; the background summarizer request sends `tools: []` (no `web_search`/`web_fetch`) | Every release touching `companion.html` |
