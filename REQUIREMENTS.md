...
| `hill-climbing-council-key` | The user's **Anthropic API key** — a credential (the first stored in the suite; browser localStorage is not a secure secret store). Used only for the user's own bring-your-own-key calls; **never synced.** | ~100 B | Until user clears or in-app "Forget key" | In-app Forget key; browser settings |
...
| `hill-climbing-companion-key` | The user's **Anthropic API key** for Companion — a credential, same class as `hill-climbing-council-key`. If unset, Companion reads `hill-climbing-council-key` as a convenience default, so one BYOK setup serves both. Device-local; **never synced.** | ~100 B | Until user clears or in-app "Remove key" | In-app Remove key; browser settings |
...
| Zero outbound network traffic at Tier 0–1 (except the opt-in flows in §1.3: Web Push, the BYOK AI apps Council / Nourish / Companion, and E2EE sync) | Network tab inspection during full session | Every release |
