...
| `hill-climbing-rooms` | Rooms world state (`rooms.html` v0.2 — the single-device prototype of the world layer): `{ v:1, onboarded, events [{ id, seq, ts, actor, room, type, payload }] }` — an **append-only, event-sourced world log** (every event is timestamped at creation; v0.2 surfaces those times — quietly + absolutely to people, more fully to agents). Every state change is an event (values, goals hierarchy, calendar, tasks, room types, rooms, roles, actors, clones, messages/turns, budget); the current world is a *reduction* over the log; old spans fold into `compaction` events (each carrying a state snapshot). **Among the most sensitive stores** — the whole self-described profile plus every message/turn — and it **grows without bound** (compaction bounds it in practice). Clones carry a frozen birth digest of the profile at an event index. **Agents author only world/room events here — never a practice store** (§1.4 intact); the per-event `actor` is authorship in a new world-layer store, outside §1.4's practice-store scope. All model calls (turns, clone chat, the moderator router, the research agent's web search) are **UI-only**, on the shared `hill-climbing-api-key`, never HC actions. Single-device: no cross-account sharing (deferred + gated). **Synced (E2EE) when sync is on** (A.5). | grows (compaction-bounded) | Until user clears (in-app "Delete everything") | In-app Delete everything; browser settings; future "delete all my data" button (Tier ≥ 2 prerequisite) |
