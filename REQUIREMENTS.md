...
- No microphone access is requested anywhere in the suite (Sing — whose opt-in on-device tuner was the only microphone use — was removed in the app-slimming pass)
...
- **Audio:** synthesised in-browser. **Microphone:** no app requests microphone access (the Sing app, whose on-device tuner was the suite's only microphone use, was removed in the app-slimming pass).
...
- **Revision history:**
  ...
  - v0.5 (inventory freshness) — added the **Sing** app (`sing.html` v0.1, founder-approved hybrid design): A.1 `hill-climbing-sing` row (ladder + songbook), A.5 `ls:hill-climbing-sing` sync blob, usage via the shared `hc-usage.js` log like the other practice apps. **First microphone use in the suite**, and deliberately bounded: the opt-in in-exercise tuner analyses the live stream on-device and discards it — never recorded, stored, or transmitted, and it is an instrument, not a judge (nothing it detects moves the ladder; outcomes stay self-reported). §2.2/§2.3/A.3/A.4 microphone lines updated from "no microphone access requested" to this disclosure. No new egress anywhere. Passes §1's three tests at Tier 0 (KNOWN_RISKS L50).
  - v0.6 (**app-slimming pass — inventory freshness**) — the founder removed four whole apps and one tab as unused: **Levity, Sing, Remind, and Govern deleted** (`levity.html`, `sing.html`, `remind.html`, `govern.html` gone; their A.1 rows and A.5 sync blobs — `hill-climbing-levity`, `hill-climbing-sing`, `hill-climbing-remind`, `hill-climbing-govern` — struck; the microphone disclosure lines in §2.2/§2.3/A.3/A.4 revert to "no microphone access requested," since Sing's tuner was the suite's only mic use), and the **Taste tab removed from Nourish** (v0.18 is cook-only; the `hill-climbing-savor` row and its A.5 blob struck — no page writes or registers the store any longer). Usage-writer lists updated to the six remaining logging apps. Data already on a user's device or in their encrypted sync rows is untouched — nothing reads it, and it deletes by the ordinary mechanisms. Removal is a *reduction* of data practices; no new binding commitment invented.
...
|---|---|---|---|---|
| `hill-climbing-usage` | Per-app usage — day-flags **plus per-day practice duration and session count**: `{ "v": 1, "meditate": { "YYYY-MM-DD": { "s": 1500, "n": 1 }, … }, "breathe": { … }, "nourish": { "YYYY-MM-DD": 1, … }, … }` (duration-bearing apps hold `{ s: total seconds, n: session count }`; flag-only apps and pre-v1.78 days hold the bare `1`). Written by the six usage-logging practice apps via the shared `hc-usage.js` — Meditate/Breathe record a completed session's seconds, the rest (Nourish, Foresee, Climb, Train) mark the day active. Read by `index.html`. This is the **this-device aggregate** (companion + offline reads); the cross-device-synced copy is the `hc-usage/records` IndexedDB store (A.2). Records seconds practised, session counts, and day-activity — **no per-session timestamps in this blob, no session content, no user-identifiable data.** | < 20 KB (grows ~5 KB/yr) | Until user clears; no auto-pruning (streak requires full history) | Browser settings; future "delete all my data" button (Tier ≥ 2 prerequisite) |
...
| `hill-climbing-nourish` | Nourish cooking-ladder state (cook-only since `nourish.html` v0.18 — the former Taste tab and its `hill-climbing-savor` store were removed in the app-slimming pass): `{ v, level, streak, cleared {id→outcome}, history [{id, outcome, ts}], active, recent[] }`. Challenge ids and self-reported outcomes only; no free text, no identity. **Synced (E2EE) when sync is on** (A.5). | < 20 KB (grows slowly) | Until user clears | Same as above |
...
- **All tiers: no audio recording, no microphone access, no behavioral telemetry, no fingerprinting, no cookies for tracking.**
...
- **Blob (last-write-wins):** `ls:hill-climbing-climb` (Climb state, v1.99), `ls:hill-climbing-companion` (Companion conversations, v0.1), `ls:hill-climbing-requests` (Companion requests to the builder, v0.9), `ls:breathe-session-duration` (Breathe session length), `ls:hill-climbing-nourish` (Nourish cooking ladder), `ls:hill-climbing-foresee` (Foresee calibration ladder + predictions, v0.1), `ls:hill-climbing-train` (Train state), `ls:hill-climbing-council` (Council deliberations), `ls:hill-climbing-reflect-photo-ai` (Reflect photo-describe consent flag, reflect v0.9 — a one-character `'1'`/`'0'` so the opt-in is decided once across devices), `ls:hill-climbing-kin` (Kin character lineage). *(Meditate no longer registers a blob — v1.80 deleted its `stillness-game` store; the deleted ERP app's `ls:hill-climbing-erp` is gone; and the app-slimming pass retired `ls:hill-climbing-levity`, `ls:hill-climbing-sing`, `ls:hill-climbing-remind`, `ls:hill-climbing-govern`, and `ls:hill-climbing-savor` — no page registers them any longer; any existing server rows persist as ordinary ciphertext until the user deletes their synced data.)*
