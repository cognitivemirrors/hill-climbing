...
| `hill-climbing-hub-suggest-seed` | Transient hand-off for the hub's "ask about this": `{ v, ts, text }` — the current suggestion + its digest, written when the user taps through to Companion and **consumed (deleted) on Companion's next load**; ignored if older than 10 min. A local write only — its content reaches Anthropic only if the user then sends a message in the seeded conversation (ordinary Companion egress). | < 8 KB, transient | Seconds — deleted on consumption; orphaned seeds overwritten by the next tap | Consumed automatically; browser settings |
