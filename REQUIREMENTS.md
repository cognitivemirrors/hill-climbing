...
- Microphone audio is never recorded or persisted (Sing's opt-in tuner, sing v0.1, analyses the live stream frame-by-frame on-device for pitch and discards it — nothing is stored or transmitted)
...
- **Audio:** synthesised in-browser. **Microphone (`sing.html` tuner only, sing v0.1):** browser-level permission required; the tuner is off until the person taps it on, the live stream is analysed on-device for pitch and discarded — never recorded, stored, or transmitted — and the mic is released when the tuner is toggled off, the exercise is left, or the tab is hidden. No other app requests microphone access.
...
| `hill-climbing-sing` | Sing voice-practice state: `{ v, level, streak, cleared {id→outcome}, history [{id, outcome, ts}], active, recent[], songbook [{title, ready, ts}] }`. `songbook[].title` is short user-entered song titles (songs being learned) — mildly personal. The in-exercise microphone tuner writes nothing here: mic audio is analysed live on-device and discarded (§2.3). **Synced (E2EE) when sync is on** (A.5). | grows slowly | Until user clears | Browser settings; future "delete all my data" button (Tier ≥ 2 prerequisite) |
...
- `sing.html` (tuner, sing v0.1): live microphone stream + a per-frame time-domain analysis buffer and a rolling window of the last few detected pitch values — analysed on-device, overwritten continuously, never persisted; the stream is released when the tuner is turned off, the exercise is left, or the tab is hidden.
...
- **All tiers: no audio recording, no behavioral telemetry, no fingerprinting, no cookies for tracking.** (Sing's opt-in tuner, sing v0.1, is the suite's only microphone use: the live stream is analysed on-device and discarded — never recorded or transmitted; see §2.3 and A.3.)
