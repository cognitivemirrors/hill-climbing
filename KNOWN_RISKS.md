...
### L32. Usage log now carries session durations **and syncs across devices** (v1.78) — binding docs amended + founder-ratified
...
- **Severity:** LOW at Tier 0. Sync is opt-in, off by default, zero-knowledge E2EE (operator can't read content). It reverses the earlier "`hill-climbing-usage` is never synced" posture, so it inherits the L28/L31 sync surface — but adds no content the journal/Climb logs don't already, and the data is the user's own practice timing that they asked to carry across their devices.
- **What changed:**
  - **Durations (v1.78, was v1.77's device-local L32):** Meditate (`endTimedSit`) and Breathe (`endBreatheSession`) record a completed session's seconds; the four flag apps mark the day active. All six now route through **one shared module `hc-usage.js`** instead of six copy-pasted inline helpers.
...
- **Binding docs amended (founder-ratified for this change):** REQUIREMENTS §1.1's `hill-climbing-usage` row now states day-flags **+ per-day duration + session count** (the "only boolean day-flags" claim is gone); new rows for `hill-climbing-usage-device`, `hill-climbing-usage-backfilled`, and the `hc-usage/records` IndexedDB store; §1.5 lists the `usage/…` log store and rewrites the "never synced" note (the localStorage aggregate stays device-local, the record log is the synced source of truth); §1.4's delete-all scope now includes the `hc-usage` database.
