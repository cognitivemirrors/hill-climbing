...
### L29. Sync extended to all apps (v2.0) — binding data-inventory now materially behind, and the Reflect journal is now off-device-capable
...
- **Severity:** LOW at Tier 0 (opt-in, off by default; zero-knowledge, so the operator still cannot read content) — but it widens the L28 surface from one store to the whole suite, so the sub-items deserve watching before any tier advance.
- **What changed:** every app with user data now registers a store. Blob (LWW): meditate `stillness-game`, breathe `breathe-session-duration`, nourish `hill-climbing-nourish`, levity `hill-climbing-levity`, train `hill-climbing-train`, council `hill-climbing-council`. Log (per-entry): the **Reflect journal** — done per L28's guidance as a per-entry merge, *not* whole-blob, with version-stamped doc keys (`id@updated`) and tombstone deletes so edits/deletions propagate under the insert-only log RPC.
- **Residual / carried risks:**
  - **Binding data-inventory is now further behind (extends L22/L26/L28).** REQUIREMENTS §1.5/§1.1 lists the sync tables and the Climb-era keys but **not** the stores added here — most importantly the **Reflect `journal` free-text**, the single most sensitive store in the suite, which now has an opt-in outbound (E2E-encrypted) path. Also missing: `stillness-game`, `breathe-session-duration`, `hill-climbing-nourish`. **These are binding-doc edits and were deliberately NOT made unilaterally — they await founder ratification** (the v1.99 amendments were ratified via plan approval; the same is needed here).
  ...
  - **Metadata leakage unchanged but broader** — the operator now also sees row counts / sizes / timestamps for six more doc namespaces (still no content). Same accepted trade-off as L28.
- **Deliberate exclusions (allowlist held):** council's Anthropic API key, nourish's Chef API key, meditate's adverse-event **reports** + trajectory recordings, and `hill-climbing-usage` (a per-day union log that blob LWW would corrupt across devices) are **not** registered and stay device-local.
- **Watch-items:** get the REQUIREMENTS §1.5/§1.1 inventory amended (founder) to name the journal and the new blob keys before any tier advance; keep credentials out of sync; if usage ever needs cross-device totals, it needs a union-merge store, not a blob.
