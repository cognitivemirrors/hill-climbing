...
### L33. Savor's seed "community" lessons are authored, not contributed — and its progression is lighter than Nourish's but still present
- **Confidence:** confirmed (design consideration, not a bug)
...
- **Binding-doc inventory (reframed by REQUIREMENTS v0.2 / L32):** under the standard-first policy, Savor is judged by the three tests, not by presence in an inventory. It passes at Tier 0: **value** — watched episodes, tasting notes, and own lessons all serve the user's own learning record, no "might-be-useful-later" data; **consent** — implicit is the Tier-0 standard, and Savor adds no *new* off-device flow (it's on-device, and joins the existing E2EE-sync flow like every practice); **control** — export/import is comprehensive and machine-readable, deletion is real. The formal per-key rows (`hill-climbing-savor` in Appendix A.1, `ls:hill-climbing-savor` in the A.5 blob list, and Savor as a new `hill-climbing-usage` writer) are a **Tier-2 deliverable** per §1/Appendix A — owed to external users, not front-loaded onto a solo dev. Adding them is a binding-doc edit (founder confirmation), flagged not self-amended. NB: the A.5 sync-blob enumeration is now factually incomplete without `ls:hill-climbing-savor` (Savor *is* a registered sync participant) — a one-line accuracy fix whenever the founder next touches Appendix A.

### L34. Savor ⇄ Nourish integration couples two files on a schema and an id contract
