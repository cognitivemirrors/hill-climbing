...
### L32. `hill-climbing-usage` now stores session duration, not just a day-flag — contradicts REQUIREMENTS §1.1's "only boolean day-flags"
- **Confidence:** confirmed (by design; meditate v1.77, breathe v0.8 — at founder request: "capture my session duration, not just whether I open the app").
- **Severity:** LOW at Tier 0. Device-local, **not** synced (the allowlist exclusion above still holds), no new egress, no new identifiers — it is more detail about the *same* user, stored on their own device, that they explicitly asked to keep. The concern is a binding-doc accuracy drift, not a user-welfare risk.
- **What changed:** at session end, meditate (`endTimedSit`) and breathe (`stopBreathwork`/`stopTraining`/`switchMode`) *upgrade* that day's value in `log[app][day]` from the bare `1` flag to a per-day aggregate `{ s: total seconds, n: session count }`. The hub (`index.html`) sums the week's `s` and shows minutes beside the day-count for Meditate/Breathe. Chosen shape is a **bounded per-day aggregate**, deliberately **not** a list of timestamped session records — that would trip the IndexedDB-migration trigger in CLAUDE.md §7 and store finer-grained behavioural traces than the founder asked for.
- **Backward/forward compatibility (verified in headless Chromium, 18 checks):** every existing reader — the hub `logDaySet`/`weekIsActive` and `companion.html`'s `readActivity` — reads only the day **keys** of `log[app]`, never the value, so mixing old `1` flags and new `{s,n}` objects is safe; both are truthy, so a day still counts as active. No new top-level key was added (that would have polluted companion's `Object.keys(log)` loop). Days logged before this change keep their `1` and contribute 0 minutes (no fake zeros).
- **Binding-doc gap (NOT self-amended):** REQUIREMENTS §1.1's `hill-climbing-usage` row still says "No session content, no user-identifiable data — **only boolean day-flags**" and estimates growth at ~3 KB/yr. That "only boolean day-flags" clause is now false, and the size grows modestly (still one small object per active app-day). **Proposed amendment for founder ratification:** update the row's value example to `… "meditate": { "YYYY-MM-DD": { "s": 1500, "n": 1 }, … }`, replace "only boolean day-flags" with "day-flags plus per-day practice duration (seconds) and session count — no timestamps, no session content," and note the still-tiny size bump. REQUIREMENTS is binding (CONSTRAINTS §6) → left for explicit founder sign-off, not applied here.
