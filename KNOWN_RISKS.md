...
- **Cost note:** each reply can now trigger up to 5 web searches + 5 fetches on the user's key (server-tool usage is billed on top of tokens). `max_uses` caps it per reply; the user owns the key and the bill, consistent with the BYOK posture.
