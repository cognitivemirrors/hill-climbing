...
- **Binding-doc tension — flagged, not silently amended (per the L24/L27/L29/L30 pattern):** REQUIREMENTS §1.1 should note that `hill-climbing-companion` now contains a persisted cross-conversation **memory summary** (not just dialogue), and §1.3/§2.1's transient-access / "nothing kept" framing for the companion needs a carve-out for it. The in-product copy is already corrected and honest; the **binding docs are left for founder ratification** rather than edited unilaterally.
