...
### L31. Savor's seed "community" lessons are authored, not contributed — and its progression is lighter than Nourish's but still present
- **Confidence:** confirmed (design consideration, not a bug)
...
- **Binding-doc inventory:** the new `hill-climbing-savor` localStorage key (and Savor as a new `hill-climbing-usage` writer) is not yet in REQUIREMENTS.md §1.1 — same class as the L21/L26 inventory gaps. Binding doc → founder ratification required; not self-amended here.

### L32. Savor ⇄ Nourish integration couples two files on a schema and an id contract
