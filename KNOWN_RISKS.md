...
### L24. Binding docs don't yet cover the planned strength app's harm class or its relaxed gamification posture
- **Confidence:** confirmed (scope gap, not a code bug; surfaced while planning the strength app)
- **Where:** CONSTRAINTS §2/§3 and REQUIREMENTS §2/§3 (safety) address only *psychological* harm; CONSTRAINTS §3.1/§5 (anti-gamification) and REQUIREMENTS §1 (data, "nothing leaves the device") are written single-practice / on-device. The planned strength app (BACKLOG → Strength app) introduces (a) a **physical-injury harm class** with zero current coverage, (b) an **intentionally relaxed gamification posture** (maximize enjoyment for capacity-building activity — a deliberate, literature-grounded departure from the meditation-era stance), and (c) **camera ML + possible off-device data** flows the current binary network claim doesn't model.
- **Impact:** none today — the strength app doesn't exist yet, and at TIER 0 (single user) the binding docs are mostly dormant scaffolding anyway (see CLAUDE.md §0). This is a *deferred-codification* marker, not a live defect. The danger is the opposite of urgency: an agent treating this as a present obligation and rewriting binding docs prematurely.
- **Direction (decided in discussion, NOT yet ratified):** allocated/asymmetric physical-safety accountability; practice-type-specific gamification within domain-independent manipulation bans; data-class × scale/tier conditional data-flow gates. Full statements in BACKLOG → "Strength app — governance directions."
- **Fix:** codify into the binding docs (founder ratification per CONSTRAINTS §6) **when approaching Tier 1 / when the strength app gets real users** — not before. Until then, leave the binding docs untouched.
