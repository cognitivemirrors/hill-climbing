...
- [x] v1.55 — Hide the camera preview thumbnail during practice. User doesn't want to see themselves mid-session. Visibility implemented as `opacity: 0 + pointer-events: none` rather than `display: none`, because the motion-detection loop reads frames from this `<video>` element via `drawImage` — browsers pause frame production for display:none / detached videos (the v1.4 bug). Element stays laid out so frames keep flowing; user just can't perceive it.
