...
- [x] **v1.90 — Levity, a fifth practice (learn to be funny).** New single-file app `levity.html` (in-product `v0.1 · levity`) built on the same 2-up/1-down staircase as Nourish: a 10-level ladder of 25 comedy-craft challenges (observation → rule of three → specifics → exaggeration → misdirection → callbacks → self-deprecation → wordplay → act-outs → building a set), each with steps, a worked example, the theory, and a "you've nailed it when" tell. Self-reported outcomes (Landed it / Got a smile / Fell flat) drive the staircase. Middle view is a **Notebook** (bit file) — capture lines from the report screen or add them by hand, star the ones that landed. Rose accent (348°) fills the warm end of the wheel past Reflect's amber. Wired into the hub: fifth card, dashboard row, `hill-climbing-levity` state key, `markUsage('levity')` into the shared usage log (keyed by the canonical `hcDayKey`), subtitle now "five practices." Export/import + corner data menu carried over.
