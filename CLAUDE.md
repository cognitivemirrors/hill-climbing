...
| `reflect.html` | Journal (`v0.7`), IndexedDB-backed (`journal`/`entries`): free-text + optional mood/satisfaction + optional **photos** (≤4/entry, compressed on-device to JPEG data URLs stored inside the entry record, so they ride the per-entry E2EE sync/export/delete unchanged — KNOWN_RISKS L41). **The most sensitive store in the suite.** | If relevant |
