...
| `foresee.html` | **Calibration training** via the staircase (`v0.1`) — the metacognition practice. Write one-line predictions about your own life with stated confidence (5–95%; exactly 50% is refused), resolve them against reality (happened / didn’t / can’t say — void never moves the ladder), Brier-score each call (**beat a coin flip = success**) into the 2-up/1-down ladder (10 levels climbing horizon *and* interiority: today-and-concrete → a-month-out inner weather). Calibration view shows a folded said-vs-happened curve — deliberately a curve, never a score (L43). First ladder graded by external reality (resolution still self-reported). Fully on-device, no egress. State: `hill-climbing-foresee`. | If relevant |
...
1. **App inventory (§2).** `ls *.html` still shows the hub + the thirteen practice apps + `anime.html`; in-product labels roughly match (`grep -h version-label *.html` / the `VERSION` constants).
