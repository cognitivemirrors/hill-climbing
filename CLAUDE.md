...
| `museum.html` | **Experiment** (`v0.2`) — two blank-slate agents, **Nat** and **Kevin**, traverse a museum of messages and photos; the archive is a museum of *the other*, so each assembles a portrait of the other and finds its own outline in the negative space. Runs **Living** (BYOK on the shared `hill-climbing-api-key`, `claude-opus-4-8` — symmetric per-being agents, one call per being per exhibit, `{say,note}`; graceful fallback) or **Dreaming** (deterministic poetic walk, no key). **You can also chat with either twin** (v0.2, "Speak with Nat/Kevin"): a bottom-sheet dialog where the being talks as itself, seeded with the portrait it assembled and framed to treat the visitor as the one it's been reconstructing — Living prose via a chat-shaped system prompt (leading scripted opener stripped so the API convo starts on a user turn), scripted in-character fallback with no key; transcripts are in-memory only, cleared on Start-over. Archive on-device only (`hill-climbing-museum`, never synced); user can add own messages/photos (photos downscaled). Hub-carded but, like `anime.html`, **not in the PWA precache/manifest** and no usage hook. | If relevant |
