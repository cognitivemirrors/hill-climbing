...
| `reflect.html` | Journal (`v0.8`), IndexedDB-backed (`journal`/`entries`): free-text + optional mood/satisfaction + optional **photos** (≤4/entry, compressed on-device to JPEG data URLs stored inside the entry record, so they ride the per-entry E2EE sync/export/delete unchanged — KNOWN_RISKS L41). Opt-in **"describe photos"** (off by default; `hill-climbing-reflect-photo-ai`) makes Reflect the **fifth BYOK surface**: each newly attached photo is sent once to Anthropic on the shared key for a description + verbatim text transcription, stored beside the photo (`desc`/`ocr`) — the suite's first image egress, L42. **The most sensitive store in the suite.** | If relevant |
