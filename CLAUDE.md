...
| `museum.html` | **Experiment** (`v0.1`) — two blank-slate agents, **Nat** and **Kevin**, traverse a museum of messages and photos; the archive is a museum of *the other*, so each assembles a portrait of the other and finds its own outline in the negative space. Runs **Living** (BYOK on the shared `hill-climbing-api-key`, `claude-opus-4-8` — symmetric per-being agents, one call per being per exhibit, `{say,note}`; graceful fallback) or **Dreaming** (deterministic poetic walk, no key). Archive on-device only (`hill-climbing-museum`, never synced); user can add own messages/photos (photos downscaled). Hub-carded but, like `anime.html`, **not in the PWA precache/manifest** and no usage hook. | If relevant |
