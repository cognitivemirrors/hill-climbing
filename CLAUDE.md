...
This is a working solo-developer project. The user is building a suite of contemplative/practice web apps under the "Hill Climbing" name (it began as a single stillness-meditation app). The suite is now **thirteen single-file practice apps and two small games behind a hub, plus one experimental webcam toy** (see §1 and §2), all at **TIER 0**. Note: §§4–5, 7–8, 10 below were largely written during the single-app era. The *collaboration norms, values, and decision history* they record are still current and load-bearing — but their file references and mechanics often describe `meditate.html` in its earlier camera-game form (see §1; that mode is currently hidden). Read those sections for norms-and-history; read §§1–3, 6, 9, 11 for the live factual state, which is what drifts.

---

## 1. Project state

> **⚠️ STRUCTURE — read before editing files.** The project is a **hub + thirteen practice apps + two games + one experiment**, all single-file, no build, no backend of our own. `index.html` is the static **hub/landing page** (practice cards + a "next step" suggestion panel — companion-written via BYOK when the opt-in toggle is on, deterministic otherwise — + weekly-usage dashboard + PWA install banner + opt-in Web Push reminder toggle). **When older prose below says "the app" or "index.html" as if it were the meditation app, it means `meditate.html`** — the meditation app was `index.html` until the multi-app split; each app links back to the hub via a top-left home link.

- **The project's intention is shifting (direction, drafted for founder ratification — nothing world-related is built; the suite is unchanged and still TIER 0).** Hill Climbing is being redrawn from a solo, on-device practice suite toward a **private, two-person, values-constrained digital world**: the practices become the world's geography and verbs (pages → "rooms"), and each person can have an **agent-character grounded in their own profile** that goes on adventures with the other. The `window.HC` parity work is its foundation — a character acts in the world indistinguishably from a human. **Turn-based first; autonomous play is the eventual destination, not day one.** The full design — a phased roadmap (Phase 0 foundations → 4 guarded autonomy), the drafted binding-doc changes it needs, and the two hard gates (security + relational-safety reviews) before partner-shared content — lives in `WORLD_ARCHITECTURE.md` and `WORLD_ROADMAP.md`; §9 tracks it as the largest open thread.
...
| `reflect.html` | Journal (`v0.9`), IndexedDB-backed (`journal`/`entries`): free-text + optional mood/satisfaction + optional **photos** (≤4/entry, compressed on-device to JPEG data URLs stored inside the entry record, so they ride the per-entry E2EE sync/export/delete unchanged — KNOWN_RISKS L41). Opt-in **"describe photos"** (off by default; `hill-climbing-reflect-photo-ai`) makes Reflect the **fifth BYOK surface**: each newly attached photo is sent once to Anthropic on the shared key for a description + verbatim text transcription, stored beside the photo (`desc`/`ocr`) — the suite's first image egress, L42. The consent is decided **once, account-wide**: the flag syncs as its own E2EE blob (`ls:hill-climbing-reflect-photo-ai`, v0.9), so enable/disable carries to every synced device. **The most sensitive store in the suite.** | If relevant |
...
| `foresee.html` | **Calibration training** via the staircase (`v0.1`) — the metacognition practice. Write one-line predictions about your own life with stated confidence (5–95%; exactly 50% is refused), resolve them against reality (happened / didn’t / can’t say — void never moves the ladder), Brier-score each call (**beat a coin flip = success**) into the 2-up/1-down ladder (10 levels climbing horizon *and* interiority: today-and-concrete → a-month-out inner weather). Calibration view shows a folded said-vs-happened curve — deliberately a curve, never a score (L44). First ladder graded by external reality (resolution still self-reported). Fully on-device, no egress. State: `hill-climbing-foresee`. | If relevant |
...
| `hc-usage.js` | Shared **usage log** (per-app day-flags + per-session practice durations). Records to a `hc-usage` IndexedDB store, keeps the legacy `hill-climbing-usage` localStorage aggregate for offline/companion, and registers a per-record `usage` sync store so totals **sum** across devices. Loaded by the nine practice apps that log usage (meditate · breathe · erp · nourish · savor · levity · foresee · climb · train) + the hub (added v1.78; Savor joined v0.3; ERP joined v0.1; Foresee joined v0.1 — marks on commit and on verdict) + **companion** (read-only, joined companion v0.11 — reads `aggregate()` so its context digest sees cross-device durations). | If touching usage/dashboard |
| `sw.js` / `manifest.webmanifest` | PWA service worker (`CACHE_VERSION` — currently `hc-v2.14`, **bump on every deploy**) + manifest. | If touching PWA |
...
- **Agent-interface watch-items (KNOWN_RISKS L45).** ERP write actions sit against the L39(d) clinical boundary (transcription-only framing lives in copy, not code); sensitive stores are now enumerable by anything in the page context; llms.txt/manifest drift is a new doc-freshness surface; REQUIREMENTS §1.4's wording awaits explicit founder read-through (the decision itself was founder-stated verbatim in-session).
...
1. **App inventory (§2).** `ls *.html` still shows the hub + the thirteen practice apps + the two games (`echo.html`, `garden.html`) + `anime.html`; in-product labels roughly match (`grep -h version-label *.html` / the `VERSION` constants).
