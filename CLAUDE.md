...
> **⚠️ STRUCTURE — read before editing files.** The project is a **hub + twelve practice apps + one experiment**, all single-file, no build, no backend of our own. `index.html` is the static **hub/landing page** (practice cards + a "next step" suggestion panel + weekly-usage dashboard + PWA install banner + opt-in Web Push reminder toggle). **When older prose below says "the app" or "index.html" as if it were the meditation app, it means `meditate.html`** — the meditation app was `index.html` until the multi-app split; each app links back to the hub via a top-left home link.
...
|---|---|---|
| `index.html` | **Hub/landing page** (~1,460 lines) — eleven practice cards (Council is reachable but not carded), a **"A next step" suggestion panel** (deterministic next-best-action over on-device data: rest / resume-in-progress / Climb focus step / time-of-day invitation; ERP deliberately excluded — KNOWN_RISKS L40), weekly-usage dashboard, PWA install banner, opt-in Web Push reminder toggle. | Yes (quick skim) |
