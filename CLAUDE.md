...
- **Off-device data flows now exist** (all opt-in, off by default): three **bring-your-own-Anthropic-key** AI apps (Council, Companion, Nourish's chef) call the Anthropic API directly on the *user's own key*; **Companion additionally has live web search/fetch**; **end-to-end-encrypted, zero-knowledge cross-device sync** (Supabase) covers every app with user data; **WHOOP health data** (Recover, v0.1) flows in via the user's own WHOOP developer app through `whoop-relay` — a stateless Supabase Edge Function, the suite's **first operator-run hop in a user-content path** (posture change, founder-accepted; KNOWN_RISKS L41) — with a compact WHOOP summary joining the Companion and hub-next-step digests when connected; and **Web Push reminders**. The whole data policy was reframed standard-first in **REQUIREMENTS §1–§2 (v0.2)** — read it before touching anything data-related.
...
|---|---|---|
| `index.html` | **Hub/landing page** (~1,680 lines) — eleven practice cards (Council is reachable but not carded), a **"A next step" suggestion panel**: an opt-in **companion-written recommendation** (BYOK on the shared `hill-climbing-api-key`, `claude-opus-4-8`, structured output, re-fetched on every return to the hub, throttled; opt-in flag `hill-climbing-hub-suggest` — the hub's only egress and the suite's first automatic-on-arrival BYOK send) layered over a **deterministic engine** (rest / low-WHOOP-recovery ease / resume-in-progress / Climb focus step / high-recovery movement invite / time-of-day invitation — recovery-aware via Recover's on-device blob since recover v0.1) that is the instant first paint and offline/error fallback; ERP deliberately excluded from both layers — KNOWN_RISKS L40. Plus weekly-usage dashboard, PWA install banner, opt-in Web Push reminder toggle. | Yes (quick skim) |
...
| `companion.html` | **BYOK AI** (`v0.12`) — **two modes of one app**, chosen by a top-centre switch that re-themes the page (teal ↔ indigo via `html[data-mode]`). **Companion**: reads your current goals + recent journal + activity (incl. summed practice durations & session counts for meditate/breathe, from the **cross-device** synced usage aggregate — v0.10/v0.11; + a compact "Their body — WHOOP, last 7 days" section when Recover is connected — v0.12) into each reply and can search/fetch the web (**broadest personal-content egress** — L29/L30); carries a cross-conversation memory (v0.8, L37); and can **write up requests about the suite to an on-device Requests list** (a `{ heading, prompt }` brief — what to build / questions to answer) via a client-side `add_request` tool, confirm-first, reviewable/copyable from menu → Requests (v0.9, L38 — on-device, no new egress; a server-side feedback service was considered and declined). **Council**: an LLM "board of directors" (4 directors + a Chair) for a decision (off-device — L24). Mode persists in `hill-climbing-cc-mode`; `?mode=council` deep-links. Three synced blobs: `hill-climbing-companion` + `hill-climbing-council` + `hill-climbing-requests`. | If relevant |
| `recover.html` | **WHOOP body-signals window** (`v0.1 · recover`) — connect your own WHOOP developer app (BYO-credentials, the BYOK shape), pull daily recovery/HRV/RHR/sleep/strain into an on-device day-map, see today + 28-day trends + week-on-week. Deliberately **not a practice**: no usage hook, no goals/streaks — a window, not a scoreboard. Requests transit the stateless `whoop-relay` Edge Function (WHOOP's API isn't browser-callable). State: `hill-climbing-whoop` (E2EE-synced) + `hill-climbing-whoop-auth` (credentials/tokens, device-local, **never synced**). See L41. | If relevant |
...
| `sw.js` / `manifest.webmanifest` | PWA service worker (`CACHE_VERSION` — currently `hc-v2.18`, **bump on every deploy**) + manifest. | If touching PWA |
| `supabase/functions/whoop-relay/` | **WHOOP relay** (Edge Function, deployed to the suite's Supabase project) — stateless pipe for Recover's OAuth token exchange + read-only v2 GETs; stores/logs nothing, upstream pinned to `api.prod.whoop.com`. The one piece of our own backend; keep the deployed function matching this source (REQUIREMENTS §5 row). | If touching Recover/WHOOP |
