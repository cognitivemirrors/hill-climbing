...
1. **App inventory (§2).** `ls *.html` still shows the hub + the twelve practice apps + the two games (`echo.html`, `garden.html`) + `anime.html`; in-product labels roughly match (`grep -h version-label *.html` / the `VERSION` constants).
