...
- **Becoming a product.** The world's values defense holds only while it is a *private artifact for two people*. A third+ participant, any public or shareable surface, any metric or record designed to grow, any onboarding funnel, or any monetization means it **has become a product**, and crossing any of these requires re-running the full values reconciliation as a fresh amendment. *"Two only" is binding; adding anyone is a product-shape change gated by re-ratification.*
  - **[RATIFIED 2026-07 — founder]** This re-ratification was **invoked and granted** for a scoped cross-user surface: **`contacts.html`** adds a contact list + human↔human messaging + a friend graph ("other users") — a product-shape change under this clause (the founder was asked explicitly and chose to cross it). **Scope + safeguards as ratified:** messaging is **human↔human and human↔your-own-agents only** — **no cross-user *portrayed* characters**, so S8's portrayal-harm class is **not** triggered by this surface; the **relational-safety gate** (S8(a)/(c)) is met by **two-sided consent (mutual accept), blocking, and exit enforced in the database** (RLS + `security definer` RPCs, `contacts-schema.sql`), not merely the UI; the **crypto gate** (WORLD_ROADMAP Phase 2) is met **honestly under the public-data frame** — messages are **public seed data, server-readable, not E2EE**, disclosed in-thread (no false confidentiality), readable only by the pair (RLS), with operator-visible metadata disclosed. **Not yet done:** live two-account verification; an abuse-report path. This records the decision; the clause itself **stands** for the next crossing — a third human beyond pairwise messaging, any public/shareable surface, or cross-user *portrayed* characters each require their own re-ratification. (REQUIREMENTS §8 v0.6; KNOWN_RISKS L50.)
